...
def btn_t(key: str, lang_code: str = 'en', **kwargs) -> str:
    """
    Button text translation helper - shorthand for button namespace
    
    This is a convenience function that automatically prefixes the key with 'buttons.'
    to access the buttons translation namespace, making button translations cleaner
    and more consistent throughout the codebase.
    
    Args:
        key: Button translation key (without 'buttons.' prefix)
        lang_code: Target language code (defaults to English)
        **kwargs: Variables for string formatting (e.g., price, days, etc.)
        
    Returns:
        Localized button text with variables substituted
    
    Examples:
        btn_t('rdp_main', 'fr')  # Returns: "💻 Windows RDP"
        btn_t('rdp_from_price', 'es', price='5.00')  # Returns: "💻 Windows RDP desde $5.00"
        btn_t('purchase_hosting', 'en', price='9.99')  # Returns: "✅ Purchase Hosting - $9.99/month"
        btn_t('renew_hosting', 'fr', days=7)  # Returns: "💳 Renouveler Hébergement (7 jours avant expiration)"
    """
    return t(f'buttons.{key}', lang_code, **kwargs)
...
async def set_user_language_preference(telegram_id: int, lang_code: str, manually_selected: bool = False) -> bool:
    """
    Set user's language preference in database and invalidate cache
    Handles race conditions by ensuring user exists before setting preference
    
    Args:
        telegram_id: Telegram user ID
        lang_code: Language code to set
        manually_selected: Whether the user manually selected this language (default: False for backward compatibility)
    ...
    from database import execute_update, get_or_create_user
    from performance_cache import performance_cache
    
    try:
        # CRITICAL FIX: For language preference updates, first try direct update for existing users
        # This is more efficient and avoids unnecessary user creation calls
        rows_affected = await execute_update(
            "UPDATE users SET preferred_language = %s, language_selected_manually = %s WHERE telegram_id = %s",
            (lang_code, manually_selected, telegram_id)
        )
        
        # If no rows were affected, the user doesn't exist - create them
        if rows_affected == 0:
            # User doesn't exist, try to create them (for new users during onboarding)
            user_record = await get_or_create_user(telegram_id)
            if not user_record:
                logger.error(f"❌ Could not create user {telegram_id} for language preference")
                return False
            
            # Try the update again after user creation
            rows_affected = await execute_update(
                "UPDATE users SET preferred_language = %s, language_selected_manually = %s WHERE telegram_id = %s",
                (lang_code, manually_selected, telegram_id)
            )
        ...
        if rows_affected > 0:
            # Invalidate cache immediately after successful DB update using the correct cache key format
            cache_key = f"user_lang:{telegram_id}"
            performance_cache.invalidate_key(cache_key, 'user_language')
            selection_type = "manually" if manually_selected else "automatically"
            logger.info(f"✅ Updated language preference for user {telegram_id}: {lang_code} ({selection_type} selected, cache invalidated)")
            return True
        else:
            logger.warning(f"⚠️ User {telegram_id} not found when setting language preference after creation attempt")
            return False
    except Exception as e:
