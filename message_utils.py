...
def render_crypto_payment(address: str, crypto_name: str, amount: Optional[str] = None, usd_amount: Optional[float] = None, crypto_amount: Optional[str] = None, memo: Optional[str] = None, order_id: Optional[str] = None, expires_minutes: int = 15, lang_code: str = 'en', payment_context: str = 'domain') -> Tuple[str, 'InlineKeyboardMarkup']:
    """
    Render cryptocurrency payment with mobile-friendly compact UI
    
    Args:
        address: Cryptocurrency address
        crypto_name: Display name of cryptocurrency (e.g., "Bitcoin", "Ethereum")
        amount: Optional formatted amount string (legacy compatibility)
        usd_amount: Optional USD amount for flexible deposits
        crypto_amount: Optional crypto amount equivalent
        ...
        payment_context: Context for routing ('domain', 'hosting', 'rdp', 'wallet')
    ...
    # Compact header
    if amount:
        # Fixed amount payment (domain purchases)
        message_parts.append(f"💳 {format_bold(crypto_name)} - {format_bold(amount)}")
    else:
        # Flexible amount payment (wallet deposits)  
        crypto_symbol = crypto_name.split()[0] if ' ' in crypto_name else crypto_name
        if usd_amount:
            if crypto_amount:
                message_parts.append(f"💳 {format_bold(crypto_name)} - ${format_bold(f'{usd_amount:.2f}')} (≈ {crypto_amount})")
            else:
                message_parts.append(f"💳 {format_bold(crypto_name)} - ${format_bold(f'{usd_amount:.2f}')}")
        else:
            message_parts.append(f"💳 {format_bold(crypto_name)} - Any Amount")
    
    # Payment address (most important info)
    message_parts.append(f"\n📬 {format_inline_code(address)}")
    
    # Memo if needed (compact format)
    if memo:
        message_parts.append(f"📋 {format_inline_code(memo)}")
    
    # Compact status line
    status_line = f"💡 Tap to copy • ⏰ {expires_minutes}min"
    message_parts.append(f"\n{status_line}")
    ...
    # QR code button (most important) - context-aware callback routing
    if order_id:
        # Use unified show_wallet_qr for all contexts (it routes correctly based on order type)
        keyboard.append([InlineKeyboardButton("📱 QR Code", callback_data=f"show_wallet_qr:{order_id}")])
    
    # Memo copy button if needed
    if memo:
        keyboard.append([InlineKeyboardButton("📋 Copy Memo", callback_data=f"copy_memo_{memo}")])
